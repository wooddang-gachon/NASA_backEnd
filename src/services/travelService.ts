...
export default class TravelService {
  ...
  public async startPlanetTravel(
    ...
    const aiReportResult = await this.aiService.generatePlanetReport(data.planetType, {
      userId,
  ...
  public async getDashboard(
    ...
    const workoutLogs = await this.travelRepository.findQuickLogsByUserCategoryAndDate(
      userId,
      'EXERCISE',
      startDate,
    );
  ...
  public async generateOndemandReport(userId: number, planetType: PlanetType = PlanetType.MEAL) {
    ...
    const travel = await this.travelRepository.createPlanetTravel({
      ...
      summary_content:
        aiReportResult.markdown || aiReportResult.findings || '탐사 진단 내용입니다.',
   ...
   * 비동기 탐사 결과 생성 (Queue)
   */
  public async generateAsyncReport(userId: number, period: 'WEEKLY' | 'MONTHLY' = 'WEEKLY') {
