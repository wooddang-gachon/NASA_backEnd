...
export default class ChatService {
  ...
  public async processChat(userId: number, userMessage: string): Promise<ChatMessageApiResponse> {
    ...
    const aiResult = await this.aiService.processChat(userId, userMessage, user.nickname, history);
    ...
    const tammyMsg = await this.chatRepository.createChatMessage(
      ChatMapper.toTammyMessageInput(
        ...
        aiResult.intentLabel || (aiResult.extractedMemory?.category ? 'MEMORY_EXTRACT' : 'CHAT'),
        aiResult.labels || (aiResult.extractedMemory ? { memory: aiResult.extractedMemory } : null),
      ),
    );
