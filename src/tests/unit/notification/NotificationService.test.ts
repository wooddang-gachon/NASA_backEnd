...
describe("NotificationService", () => {
  ...
  describe("registerPushToken", () => {
    it("should register push token with default IOS type if deviceType is not provided", async () => {
      mockNotificationRepository.upsertPushToken.mockResolvedValue({} as any);

      const result = await service.registerPushToken(1, { deviceToken: "token-123" } as any);

      expect(mockNotificationRepository.upsertPushToken).toHaveBeenCalledWith(1, "token-123", "IOS");
    ...
    it("should register push token with provided deviceType", async () => {
      ...
      const result = await service.registerPushToken(1, { deviceToken: "token-123", deviceType: "ANDROID" } as any);

      expect(mockNotificationRepository.upsertPushToken).toHaveBeenCalledWith(1, "token-123", "ANDROID");
    });
  });
  ...
  describe("sendPushNotification", () => {
    it("should return false if no active tokens found", async () => {
      ...
      expect(mockNotificationRepository.findActivePushTokens).toHaveBeenCalledWith(1);
    });
    ...
    it("should send notification successfully to all active tokens", async () => {
      ...
      const multicastSpy = jest.spyOn(service, "sendMulticastPushNotification").mockResolvedValue({ successCount: 2, failureCount: 0 });
      ...
      expect(multicastSpy).toHaveBeenCalledWith(["token1", "token2"], "Test", "Message", { key: "value" });
    });
  });

  describe("sendMulticastPushNotification", () => {
    it("should return immediately if deviceTokens array is empty", async () => {
      const result = await service.sendMulticastPushNotification([], "Title", "Body");
    ...
    it("should simulate send in sandbox mode if Firebase not initialized", async () => {
      ...
      const result = await service.sendMulticastPushNotification(tokens, "Title", "Body");
