...
describe("ChatService", () => {
  ...
  describe("processChat", () => {
    it("should throw UserNotFoundError if user not found", async () => {
      ...
      await expect(chatService.processChat(1, "Hi")).rejects.toThrow(UserNotFoundError);
    });

    it("should process chat and return response", async () => {
      mockChatRepository.findUserById.mockResolvedValue({ nickname: "test" } as any);
      mockChatRepository.createChatMessage.mockResolvedValue({} as any);
      mockChatRepository.findRecentChatMessages.mockResolvedValue([
        { sender: Sender.USER, message_text: "Hi", created_at: new Date() } as any
      ]);
      mockChatRepository.updateUserFuel.mockResolvedValue({ current_fuel: 10 } as any);
      ...
      expect(mockAiService.processChat).toHaveBeenCalledWith(1, "Hi", "test", expect.any(Array));
    ...
    it("should handle error in finding recent chat messages", async () => {
      mockChatRepository.findUserById.mockResolvedValue({ nickname: "test" } as any);
      mockChatRepository.createChatMessage.mockResolvedValue({} as any);
      mockChatRepository.findRecentChatMessages.mockRejectedValue(new Error("DB Error"));
      mockChatRepository.updateUserFuel.mockResolvedValue({ current_fuel: 10 } as any);
      ...
      expect(result).toBeDefined();
      expect(mockAiService.processChat).toHaveBeenCalledWith(1, "Hi", "test", []);
    });
  });

  describe("deleteMessage", () => {
    it("should update message deleted state to true", async () => {
      mockChatRepository.updateMessageDeletedState.mockResolvedValue({} as any);

      await chatService.deleteMessage("1");

      expect(mockChatRepository.updateMessageDeletedState).toHaveBeenCalledWith(BigInt(1), true);
    });
  });
  ...
  describe("undoDeleteMessage", () => {
    it("should update message deleted state to false", async () => {
      ...
      expect(mockChatRepository.updateMessageDeletedState).toHaveBeenCalledWith(BigInt(1), false);
    });
  });
});
