...
describe("FoodService", () => {
  ...
  describe("uploadAndAnalyzeFoodVision", () => {
    it("should throw error if file is missing", async () => {
      await expect(foodService.uploadAndAnalyzeFoodVision(undefined)).rejects.toThrow("업로드할 이미지 파일(file)이 누락되었습니다.");
    });
  ...
  describe("logMeal", () => {
    it("should throw UserNotFoundError if user does not exist", async () => {
      ...
      await expect(foodService.logMeal(999, { mealType: "LUNCH" } as any)).rejects.toThrow(UserNotFoundError);
    });
  ...
  describe("searchFoods", () => {
    it("should search foods by keyword and return mapped response", async () => {
      mockFoodRepository.searchFoodsByKeyword.mockResolvedValue([
        { id: 1, name: "Apple", manufacturer: "None", calories_kcal: 50, carbohydrate_g: 10, protein_g: 0, fat_g: 0, category: "Fruit", standard_serving_g: 100 }
      ] as any);
