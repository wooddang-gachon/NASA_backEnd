...
export default async ({ expressApp }: { expressApp: express.Application }) => {
    ...
    if (config.nodeEnv === "test" || process.env.USE_MOCK === "true") {
      ...
      const MockAiService = (await import("../services/mocks/mockAiService")).default;
      const AuthService = (await import("../services/authService")).default;
      const MockAuthService = (await import("../services/mocks/mockAuthService")).default;
      
      Container.set(AiService, Container.get(MockAiService));
      Container.set(AuthService, Container.get(MockAuthService));
      Logger.info('⚠️ Test/Mock mode enabled: Injected Mock services for AI and Auth.');
    }
