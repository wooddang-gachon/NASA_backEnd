...
erDiagram
    ...
    users ||--o{ reports : "맞춤 AI 별여행 탐사 결과(travelResult)"
    ...
    planet_travels ||--o| reports : "1:1 탐사 결과(travelResult)"
...
#### 2) `reports` (별여행 탐사 결과 `travelResult` 마스터)

| 컬럼명             | 데이터 타입        | 제약 조건                                | 설명                       |
| :----------------- | :----------------- | :--------------------------------------- | :------------------------- |
| `id`               | BIGINT             | PK, AUTO_INCREMENT                       | 탐사 결과 ID (`travelResultId`) |
| `user_id`          | INT                | FK (users.id, NOT NULL)                  | 사용자 ID                  |
| `planet_travel_id` | BIGINT             | UNIQUE, NULLABLE, FK (planet_travels.id) | 1:1 탐사 연동              |
| `planet_type`      | ENUM(`PlanetType`) | NOT NULL                                 | 행성 테마                  |
| `title`            | VARCHAR(255)       | NOT NULL                                 | 탐사 결과 제목             |
| `summary_content`  | TEXT               | NOT NULL                                 | AI 진단 요약문             |
| `recommendations`  | TEXT               | NOT NULL                                 | 맞춤 추천 가이드라인       |
| `created_at`       | TIMESTAMP          | DEFAULT CURRENT_TIMESTAMP                | 생성 일시                  |
